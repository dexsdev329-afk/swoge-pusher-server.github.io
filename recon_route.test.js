'use strict';
/* ============================================================================
 * /recon — UNE PORTE PUBLIQUE QUI VA CHERCHER CHEZ LES AUTRES
 *
 * Les gardes de fond vivent dans `recon.js` et sont mesurees par
 * `recon.test.js`. Ce qui se joue ICI est propre a la route, et c est ce qui
...
const BAC = fs.mkdtempSync(path.join(os.tmpdir(), 'recon-'));
...
(async () => {
  ...
  console.log('-- ce qui n est pas un domaine n entre pas, et ne coute pas une place --');
  {
    ...
    for (const mauvais of ['', 'Jean Dupont', 'jean.dupont@acme.io', '+33612345678', '0612345678',
      ...
      const r = await lit('/recon?d=' + encodeURIComponent(mauvais));
  ...
  console.log('\n-- un releve sort, et il est partageable --');
  {
    ...
    await lit('/recon?d=' + encodeURIComponent('https://WWW.ACME.IO/quoi'));
    await lit('/recon/acme.io.');
  ...
  console.log('\n-- un domaine qui pointe vers le reseau interne n est pas visite --');
  {
    ...
    APPELS.length = 0;
    const r = await lit('/recon/interne.io');
  ...
  console.log('\n-- la porte est publique, et elle est etroite --');
  {
    ...
    const i = src.indexOf("path === '/recon'");
    ...
    ok(bloc.indexOf('normaliseDomaine') < bloc.indexOf('reconDebit'), 'et elle est jugee AVANT le debit');
    ...
    /* Aucune variante « par personne » n existe dans le serveur. */
    ok(!/path\.startsWith\('\/(osint|identite|personne|people|whois-person)/.test(src),
  ...
  console.log('\n-- le debit borne ce qu on peut infliger a un site tiers --');
  {
    ...
    for (let i = 0; i < 8; i++) {
      const r = await lit('/recon/d' + i + '.example');
    ...
    const r = await lit('/recon/d0.example');
